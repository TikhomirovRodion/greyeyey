...
def click_btn1():
    ...
    def click_btnAnsw():
        ...
        if float(answU1) == round(U1, 2):
            label1 = ttk.Label(text='Да')
            label1.place(x = 700, y = 450)
        else:
            label1 = ttk.Label(text='Нет')
            label1.place(x=700, y=450)

        if float(answU2) == round(U2, 2):
            canvas1.create_text(675, 490, text='Да')
        else:
            canvas1.create_text(675, 490, text='Нет')

        if float(answU3) == round(U3, 2):
            canvas1.create_text(675, 515, text='Да')
        else:
            canvas1.create_text(675, 515, text='Нет')

        if float(answI) == round(I, 2):
            canvas1.create_text(675, 540, text='Да')
        else:
            canvas1.create_text(675, 540, text='Нет')
